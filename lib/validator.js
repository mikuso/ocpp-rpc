...
class Validator {
    ...
    validate(schemaId, params) {
        ...
        if (!validator) {
            throw createRPCError("InternalError", `Schema '${schemaId}' is missing from subprotocol schema '${this._subprotocol}'`);
        }
        ...
        if (!res && validator.errors?.length > 0) {
            ...
            throw createRPCError(rpcErrorCode, this._ajv.errorsText(validator.errors), {errors: validator.errors});
        }
