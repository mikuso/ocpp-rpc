...
export class RPCServer extends EventEmitter {
    ...
    constructor(options) {
        ...
        this._wss.on('error', err => this._logError(err));
     ...
     * @param {(cli: RPCServerClient) => void} callback 
     */
    ...
    get handleUpgrade() {
         ...
         * @param {import('node:net').Socket} socket - Network socket between the server and client
                ...
                const accept = (session, protocol) => {
                    ...
                    try {
                        ...
                        if (protocol !== undefined) {
                            // ensure selected protocol is known by this server
                            if (!protocols.has(protocol)) {
                                throw new WebsocketUpgradeError(400, `Client doesn't support expected subprotocol`);
                            }
                        } else {
                            // no protocol selected? pick first subprotocol (preferred by server) that is also supported by the client
    ...
    async _onConnection(websocket, request) {
        try {
            ...
            this._protocolClientHandlers.get(client.protocol)?.(client);
    ...
    _logError(err) {
        if (this.listenerCount('error') > 0) {
            ...
            console.error('[Server Warning] Unhandled error:', err);
        }
    }
}
