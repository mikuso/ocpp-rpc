...
const { TimeoutError, MalformedMessageError, UnexpectedHttpResponse, RPCNotImplementedError, RPCFrameworkError, RPCGenericError } = require('./errors');
...
class RPCClient extends EventEmitter {
    ...
    async connect() {
        ...
        this._connectionUrl = this._options.endpoint + '/' + encodeURIComponent(this._options.identity);
    ...
    _onMessage(buffer) {
        try {
            ...
            const [messageType, msgId, ...more] = payload;
            ...
            switch (messageType) {
                ...
                case MSG_CALLRESULT:
                    const [result] = more;
                    this.emit('result', {outbound: false, payload});
    ...
    async _onCall(msgId, method, params) {
        try {
            ...
            this.emit('result', {outbound: true, payload});
            this.sendRaw(JSON.stringify(payload));
        } catch (err) {
