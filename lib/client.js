...
const { TimeoutError, UnexpectedHttpResponse, RPCNotImplementedError, RPCFrameworkError, RPCGenericError } = require('./errors');
...
class RPCClient extends EventEmitter {
    ...
    useSchemaValidator(schemaValidator) {
        if (typeof schemaValidator === 'string') {
            this._schemaValidator = schemas[schemaValidator ?? this._protocol];
        } else {
    ...
    async connect() {
        ...
        if (!('identity' in this._options)) {
            throw Error("'identity' option is required");
        }
    ...
    _beginConnect() {
        this._connectPromise = (async () => {
            ...
            const headers = Object.assign({}, this._options.headers);
    ...
    _onMessage(buffer) {
        ...
        try {
            ...
            try {
                ...
                throw new RPCFrameworkError("Message must be a JSON structure");
            }

            if (!Array.isArray(payload)) {
                throw new RPCFrameworkError("Message must be an array");
            }

            const [messageType, msgIdPart, ...more] = payload;
            if (typeof msgIdPart !== 'string') {
                throw new RPCFrameworkError("Message ID must be a string");
            }
            ...
            this.emit('badMessage', buffer);

            if (++this._badMessagesCount > this._options.maxBadMessages) {
                this.close({
                    code: 1002,
                    reason: (err instanceof RPCGenericError) ? err.message : "Protocol error"
                });
            } else {
        ...
        try {
            ...
            try {
                if (this._pendingResponses.has(msgId)) {
                    throw Error(`Already processing a call with ID: ${msgId}`);
                }
                ...
                if (this._schemaValidator) {
                    this._schemaValidator(`urn:${method}.req`, params);
                }
                ...
                if (rpcErrorCode === 'FormatViolation' && this.protocol === 'ocpp1.6') {
                    // fix an error in the spec (clarified in 5.2 of OCPP1.6J errata v1.0)
