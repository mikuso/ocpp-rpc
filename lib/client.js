...
const { TimeoutError, MalformedMessageError, UnexpectedHttpResponse, RPCNotImplementedError, createRPCError } = require('./errors');
const { getErrorPlainObject } = require('./util');
...
class RPCClient extends EventEmitter {
    constructor(options) {
        ...
        this._options = Object.assign({
            ...
            protocols: [],
            reconnect: false,
            reconnectWithoutSubprotocol: false,
    ...
    async close({code, reason, awaitPending, force} = {}) {
        ...
        if (this._state === OPEN) {
            ...
            this._state = CLOSED;
            // abort always returns code 1001
            const result = {code: 1001, reason: "Connection aborted"};
    ...
    _attachWebsocket(ws, leadMsgBuffer) {
        ...
        leadMsgBuffer.forEach(msg => this._onMessage(msg));
    }
    ...
    _handleDisconnect({code, reason}) {
        ...
        const shouldReconnect = this._state !== CLOSING
            && this._options.reconnect
            && (this._options.reconnectWithoutSubprotocol || !!this._protocolOptions?.length);
    ...
    _beginConnect() {
        this._connectPromise = (async () => {
            ...
            this._ws.on('message', addToMsgBuffer);
            ...
            try {
                ...
                this._ws.off('message', addToMsgBuffer);
