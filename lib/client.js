...
const { TimeoutError, MalformedMessageError } = require('./errors');
...
class RPCClient extends EventEmitter {
    ...
    _tryReconnect() {
        return this._beginConnect().catch(async (err) => {
            this._reconnectAttempt++;
            if (this._reconnectAttempt >= this._options.maxReconnects) {
                // give up
                this.close({code: 1001});
            } else {
                try {
    ...
    async _onCall(msgId, method, params) {
        try {
            ...
            try {
                ...
                payload = [MSG_CALLERROR, msgId, err.code, err.message, err.details || {}];
            }
    ...
    _onCallError(msgId, errorCode, errorDescription, errorDetails) {
        const pendingCall = this._pendingCalls.get(msgId);
        if (pendingCall) {
            const err = Error(errorDescription);
            err.code = errorCode;
