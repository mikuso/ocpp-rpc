...
const { TimeoutError, MalformedMessageError, UnexpectedHttpResponse, RPCNotImplementedError, RPCFrameworkError } = require('./errors');
...
class RPCClient extends EventEmitter {
    ...
    _onMessage(buffer) {
        try {
            ...
            this.close({
                code: 1002,
                reason: (err instanceof RPCFrameworkError) ? err.message : "Protocol error"
            });
        }
    }
    ...
    async _onCall(msgId, method, params) {
        try {
            ...
            try {
                ...
                this._pendingResponses.add(msgId, pending);
