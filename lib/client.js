...
import { getErrorPlainObject, createRPCError, getPackageIdent, ConnectionState } from './util.js';
...
export const MessageType = Object.freeze({
    ...
    MSG_CALLRESULT: 3,
    MSG_CALLERROR: 4,
    MSG_CALLRESULTERROR: 5,
    MSG_SEND: 6,
});

/**
 * @typedef {[MessageType.MSG_CALL, string, string, Record<string, *>]} MsgCall
 * Represents the content of a CALL message. (Message Type Id 2)
 */
/**
 * @typedef {[MessageType.MSG_CALLRESULT, string, Record<string, *>]} MsgCallResult
 * Represents the content of a CALLRESULT message. (Message Type Id 3)
 */

/**
 * @typedef {[MessageType.MSG_CALLERROR, string, string, string, Record<string, *>]} MsgCallError
 * Represents the content of a CALLERROR message. (Message Type Id 4)
 */

/**
 * @typedef {[MessageType.MSG_CALLRESULTERROR, string, string, string, Record<string, *>]} MsgCallResultError
 * Represents the content of a CALLRESULTERROR message. (Message Type Id 5)
 */

/**
 * @typedef {[MessageType.MSG_SEND, string, string, Record<string, *>]} MsgSend
 ...
 * @prop {MessageType.MSG_CALL | MessageType.MSG_SEND} typeId The type of message being handled.
 ...
 * @prop {string} [endpoint] The RPC server's endpoint (a websocket URL). (Note: If this is not provided in the options passed to the RPCClient constructor, it must be provided with a `reconfigure()`, or passed directly to `connect()`)
 * @prop {string} [identity] The RPC client's identity. Will be automatically percent-encoded when used in the connection URL. (Note: If this is not provided in the options passed to the RPCClient constructor, it must be provided with a `reconfigure()`, or passed directly to `connect()`)
 ...
 * @prop {ConstructorParameters<typeof URLSearchParams>[0]} [query=''] An optional query string or object to append as the query string of the connection URL.
 ...
 * @prop {number} [maxBadMessages=Infinity] The maximum number of non-conforming RPC messages which can be tolerated before the client is automatically closed. The client will not automatically reconnect after closing in this way (even if `reconnect: true` was set in the constructor options.)
...
export class RPCClient extends EventEmitter {
     ...
     * Useful for logging or debugging. To handle and respond to calls, use client.handle() instead.
     * @event
     * @overload
     * @param {'message'} event
     ...
     * @overload
     * @param {'call'} event
     * @param {(data: {messageId: string, outbound: boolean, payload: MsgCall}) => void} listener
     * @returns {this}
     */
    /**
     ...
     */
    /**
     * Emitted immediately before an outbound response is sent, or immediately before an inbound response is processed. Useful for logging or debugging.
     * @deprecated This event has been deprecated and will be removed in a future version. Prefer to listen to the `'callResult'`, `'callError'` and `'callResultError'` events instead.
     * @event
     * @overload
     ...
     */
    /**
     * The mutually agreed websocket subprotocol, determined at connection time. Emitted when the client protocol has been set.
     ...
     * @param {(data: {error: Error, typeId: typeof MessageType[keyof typeof MessageType], messageId: string, method: string, params: Record<string, any>, result: Record<string, any>|null, outbound: boolean}) => void} listener
     * @returns {this}
     */
    /**
     * @overload
    ...
    constructor(options) {
        ...
        /** @internal @type {ConnectionStateType} */
     ...
     *
     * @param {string} [endpoint] The RPC server's endpoint (a websocket URL). If not provided, then it is assumed that the endpoint was already set in the RPCClient constructor or by a call to `reconfigure()`. Without an endpoint, `connect()` will throw.
     * @param {string} [identity] The RPC client's identity. Will be automatically percent-encoded when used in the connection URL. If not provided, then it is assumed that the identity was already set in the RPCClient constructor or by a call to `reconfigure()`. Without an identity, `connect()` will throw.
     * @param {ConstructorParameters<typeof URLSearchParams>[0]} [query] An optional query string or object to append as the query string of the connection URL.
     ...
     */
    async connect(endpoint, identity, query) {

        endpoint ??= this._options.endpoint;
        identity ??= this._options.identity;
        ...
        let connUrl = endpoint + '/' + encodeURIComponent(identity);
        if (query) {
    ...
    /**
     * Sends arbitrary data across the websocket. Not to be confused with the OCPP `SEND` message type (see `send()` instead.) Not intended for general use.
     * 
     ...
     * @param {string | Buffer | DataView | number | ArrayBufferView | Uint8Array | ArrayBuffer | SharedArrayBuffer | Blob} message - A raw message to send across the WebSocket.
     */
    sendRaw(message) {
        if ([ConnectionState.OPEN, ConnectionState.CLOSING].includes(this._state) && this._ws) {
     ...
     * 
     * A connection closed by this method will not automatically reconnect, even if `reconnect: true`
     * was set in the constructor options.
     *
     * The final resolved close code & reason may differ from what was passed to this method:
    ...
    async close({code, reason, awaitPending, force} = {}) {
        if ([ConnectionState.CLOSED, ConnectionState.CLOSING].includes(this._state)) {
            // no-op
    ...
    /**
     * Sends a `SEND` RPC message to the server. The `SEND` message type was introduced in OCPP 2.1. It acts similar to a `CALL` message, except it does not expect a response.
     * @param {string} method 
     * @param {object} [params] 
     * @returns {void}
     */
    send(method, params = {}) {
        ...
        /** @type {MsgSend} */
        const payload = [MessageType.MSG_SEND, msgId, method, params];
    ...
    /**
     * Sends a `CALL` message to trigger a remote call handler and wait for a response.
     *
     ...
     * communications permanently once `callConcurrency` is exhausted if the remote party never responds.
     *
     * @param {string} method - The RPC method to call.
     * @param {Record<string, any>} [params] - A value to be passed as params to the remote handler.
    ...
    async _call(method, params, options = {}) {
        ...
        const msgId = randomUUID();
        /** @type {MsgCall} */
        const payload = [MessageType.MSG_CALL, msgId, method, params];
        ...
        if (this._strictProtocols.includes(this._protocol)) {
            ...
            try {
                ...
                if (error instanceof RPCError) {
                    error.asOCPP16Deprecated(this._featureset.enableOcpp16DeprecatedErrorCodes);
                }

                this.emit('strictValidationFailure', {
                    messageId: msgId,
                    typeId: MessageType.MSG_CALL,
                    method,
                    params,
                    result: null,
                    error,
        ...
        // TODO: unify this event with callResult and callError, ensuring the payload and parsed params are available in all events
        ...
        try {
            ...
            if (err instanceof RPCError) {
                err.asOCPP16Deprecated(this._featureset.enableOcpp16DeprecatedErrorCodes);
            }
    ...
    _handleDisconnect({code, reason}) {
        ...
        if (this._state !== ConnectionState.CLOSING && this._options.reconnect) {
    ...
    async _onMessage(buffer) {
        ...
        try {
            ...
            messageType = /** @type {typeof MessageType[keyof typeof MessageType]} */ (messageTypePart);
            ...
            switch (messageType) {
                ...
                case MessageType.MSG_CALLRESULT: {
                    const [result] = more;
                    // TODO: remove the "response" event entirely, sending only callResult, callError, or callResultError events instead.
                    this.emit('response', {outbound: false, payload});
                    this._onCallResult(msgId, result);
                    break;
                }
                case MessageType.MSG_CALLERROR: {
                ...
                case MessageType.MSG_CALLRESULTERROR: {
                    ...
                    // TODO: remove the "response" event entirely, sending only callResult, callError, or callResultError events instead.
                    this.emit('response', {outbound: false, payload});
                    // TODO: CALLRESULTERROR is not covered in the test suite. Once added, review the error handling guide to make sure it's still consistent with the tested behaviour.
            ...
            const rpcErrorCode = (error instanceof RPCError)
                ? error.asOCPP16Deprecated(this._featureset.enableOcpp16DeprecatedErrorCodes)
                : 'GenericError';
            ...
            switch (messageType) {
                ...
                case MessageType.MSG_CALLRESULT: {
                    ...
                    response = [
                        MessageType.MSG_CALLRESULTERROR,
                        msgId,
                        rpcErrorCode,
                        errorMessage,
                        details ?? {},
            ...
            if (response && [ConnectionState.OPEN, ConnectionState.CLOSING].includes(this._state)) {
    ...
    async _onCall(msgId, method, params) {
        ...
        try {
            ...
            if (!([ConnectionState.OPEN, ConnectionState.CONNECTING].includes(this._state))) {
            ...
            payload = [MessageType.MSG_CALLRESULT, msgId, result];
            ...
            if (this._strictProtocols.includes(this._protocol)) {
                ...
                try {
                    ...
                    this.emit('strictValidationFailure', {
                        messageId: msgId,
                        typeId: MessageType.MSG_CALLRESULT,
                        method,
                        params,
                        result,
                        error,
