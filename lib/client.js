...
const { TimeoutError, UnexpectedHttpResponse, RPCFrameworkError, RPCGenericError, RPCMessageTypeNotSupportedError, RequestValidationError, ResponseValidationError } = require('./errors');
    ...
    async _call(method, params, options = {}) {
        ...
        if (this._strictProtocols.includes(this._protocol)) {
            ...
            try {
                ...
                const err = new RequestValidationError(`Call '${method}' failed validation`);
                err.data = params;
                err.error = error;
                throw err;
            }
        ...
        if (pendingCall) {
            ...
            if (this._strictProtocols.includes(this._protocol)) {
                ...
                try {
                    ...
                    const err = new ResponseValidationError(`Response for call '${pendingCall.method}' failed validation`);
                    err.data = result;
                    err.error = error;
                    return pendingCall.reject(err);
                }
            }
